...
public class Reducer {
	public static class OutputStructure {
	    ...
	    public OutputStructure (long t_id, String c, String t, long id, long s, long fcount) {
	        this.tweet_id = t_id;
	        this.created_at = c;
	        this.text = t;
	        this.user_id = id;
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if( reduceResult == null)
					continue;
				long sentimentScore = reduceResult.score * ( 1 + mapJson.getFollowersCount() );
				String reduceText = reduceResult.text.replace(SECRET_CODE, "\n");
				
				OutputStructure output = new OutputStructure(mapJson.getTweetId(), mapJson.getCreatedAt(), reduceText, mapJson.getUserId(), sentimentScore, mapJson.getFollowersCount());				
